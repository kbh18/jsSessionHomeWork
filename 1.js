...
const getStoreFruitsAvg = (store) => {
    const prices= Object.values(store.fruits).map(fruit => fruit.price)
    const sum = (acc,cur) => (acc + cur)
    const pricesSum = prices.reduce(sum)
    const avg = pricesSum/prices.length;
    return avg;
...
const findShortFruit = (fruit,location) => {
    const storesHaveFruit = stores.filter(store => hasFruit(store,fruit))
...
const allBuyFruit= (buy,store) =>{
    const buylist = Object.keys(buy)
    const buyAmount  = Object.values(buy)
    for(var i=0; i<buylist.length; i++) {
        if(!store.fruits[buylist[i]]) return "못팜"  
        else if(store.fruits[buylist[i]].stock -= buyAmount) return buyAmount+"개 구입" 
        }
}
